                ...
                )
{
    ...
    {
        int temp = 0;//记录循环次数
        for (int i = 0; i <= INPUT_DATA_W - KERNEL_SIZE_W - 1; i = i + stride)//0-46,i会循环16次最大i=45
        {
            Num_types sum = 0;
            //开始卷积操作
            for (int j = 0; j < INPUT_DATA_CHL; j++) //进行卷积操作
            {
                ...
                {
                    sum = sum + (input_data[j][i+h] * conv_2_weight[k][j][h]);
                }
            }
            sum = sum + conv_2_bias[k];//加上卷积核的bias
            output[k][temp] = ReLu(sum);
            temp++;
        }
    }
}
